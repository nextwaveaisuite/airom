...
export default function Chat() {
  ...
  const [attachments,   setAttachments]   = useState([]) // { name, type, mediaType, data, preview }
  ...
  async function createConversation(firstMessage) {
    ...
    if (data) { setActiveConvId(data.id); setConversations(c => [data, ...c]); return data.id }
    return null
  }
  ...
  async function processFile(file) {
    if (file.size > MAX_FILE_MB * 1024 * 1024) {
      setError(`File too large. Max size is ${MAX_FILE_MB}MB.`); return
    }
  ...
  function onFileInput(e) { if (e.target.files?.length) handleFiles(e.target.files) }
  ...
  async function send(text) {
    ...
    const displayText = sentAttachments.length
      ? (userText ? userText : '') + (sentAttachments.map(a => `\n📎 ${a.name}`).join(''))
      : userText

    setMessages(m => [...m, {
      role: 'user', text: displayText, id: Date.now(),
      attachments: sentAttachments.filter(a => a.preview).map(a => a.preview)
    }])
    ...
    if (!convId) convId = await createConversation(userText || sentAttachments[0]?.name || 'File analysis')
  ...
  return (
    <div style={s.shell} onDragOver={e => { e.preventDefault(); setDragOver(true) }} onDragLeave={() => setDragOver(false)} onDrop={onDrop}>
      ...
      {/* Hidden file input */}
      <input ref={fileRef} type="file" multiple accept={ACCEPTED_FILES + ',image/*'} style={{ display: 'none' }} onChange={onFileInput}/>
        ...
        <div style={s.sidebarFooter}>
          <div style={s.userRow}>
            ...
            <div>
              ...
              <div style={s.userPlan}>{profile?.plan || 'free'} · {credits} credits</div>
            </div>
          </div>
          ...
          {showWelcome && !loading && (
            <div style={s.welcome}>
              ...
              <p style={{ fontSize:12, color:'var(--text-muted)', marginTop:20 }}>
                💡 You can drag & drop files anywhere, or click the 📎 button below
              </p>
            </div>
          )}
            ...
            <div key={m.id} style={{ ...s.msgRow, justifyContent: m.role === 'user' ? 'flex-end' : 'flex-start' }}>
              ...
              <div style={{ maxWidth:'78%', display:'flex', flexDirection:'column', gap:6, alignItems: m.role==='user' ? 'flex-end' : 'flex-start' }}>
                ...
                {m.attachments?.map((src, i) => (
                  <img key={i} src={src} alt="attachment" style={{ maxWidth:240, maxHeight:180, borderRadius:10, border:'1px solid var(--border)', objectFit:'cover' }}/>
                ))}
          ...
          <div style={s.attachmentRow}>
            {attachments.map((att, i) => (
              <div key={i} style={s.attachmentChip}>
                {att.preview
                  ? <img src={att.preview} alt={att.name} style={{ width:32, height:32, borderRadius:4, objectFit:'cover' }}/>
                  : <span style={{ fontSize:18 }}>{att.mediaType === 'application/pdf' ? '📄' : '📎'}</span>
                }
                ...
                <span style={{ fontSize:11, color:'var(--text-muted)' }}>{(att.size/1024).toFixed(0)}kb</span>
                <button style={{ background:'none', border:'none', color:'var(--text-muted)', cursor:'pointer', fontSize:14, padding:'0 2px', marginLeft:'auto' }} onClick={() => removeAttachment(i)}>✕</button>
              </div>
            ))}
          </div>
        )}
        ...
        <div style={s.inputWrap}>
          <div style={s.inputRow}>
            ...
            <textarea
              ...
              onKeyDown={onKeyDown}
              placeholder={credits<=0 ? 'No credits — top up to continue' : attachments.length > 0 ? 'Add a message or just send the file…' : 'Message Airom… or drag & drop a file'}
              rows={1}
          ...
          <div style={s.inputHint}>Enter to send · Shift+Enter for new line · 📎 to attach files · drag & drop anywhere · {credits} credits left</div>
        </div>
      </div>
    </div>
  )
...
const s = {
  ...
  attachBtn:      { width:40, height:40, borderRadius:10, background:'var(--bg-hover)', border:'1px solid var(--border)', fontSize:18, cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center', flexShrink:0, color:'var(--text-secondary)' },
