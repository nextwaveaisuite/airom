...
export default function Admin() {
  ...
  const [newUser,     setNewUser]     = useState({ fullName:'', email:'', password:'', plan:'free' })
  ...
  async function createUser() {
    if (!newUser.fullName.trim() || !newUser.email.trim() || newUser.password.length < 8) {
      showToast('Please fill in all fields. Password must be 8+ characters.', 'error'); return
    }
    setAddingUser(true)
    try {
      const res  = await fetch('/.netlify/functions/admin-create-user', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ adminUserId: user.id, fullName: newUser.fullName, email: newUser.email, password: newUser.password, plan: newUser.plan })
      })
      const data = await res.json()
      if (!res.ok) { showToast(data.error || 'Failed to create user', 'error'); setAddingUser(false); return }
      showToast(data.message)
      setShowAddUser(false)
      setNewUser({ fullName:'', email:'', password:'', plan:'free' })
      loadUsers(); loadStats()
  ...
  return (
    <div style={s.shell}>
      ...
      {/* Add User Modal */}
      {showAddUser && (
        <div style={s.modalOverlay} onClick={() => setShowAddUser(false)}>
          <div style={s.modal} onClick={e => e.stopPropagation()}>
            <h3 style={{ fontSize:18, fontWeight:700, color:'var(--text-primary)', marginBottom:4 }}>Add new user</h3>
            <p style={{ fontSize:13, color:'var(--text-muted)', marginBottom:20 }}>Create a user account directly. They can log in immediately.</p>

            <div style={{ marginBottom:12 }}>
              <label style={s.label}>Full name</label>
              <input style={s.input} placeholder="Jane Smith" value={newUser.fullName} onChange={e => setNewUser(u => ({...u, fullName: e.target.value}))}/>
            </div>
            <div style={{ marginBottom:12 }}>
              <label style={s.label}>Email address</label>
              <input style={s.input} type="email" placeholder="jane@example.com" value={newUser.email} onChange={e => setNewUser(u => ({...u, email: e.target.value}))}/>
            </div>
            <div style={{ marginBottom:12 }}>
              <label style={s.label}>Password (min. 8 characters)</label>
              <input style={s.input} type="password" placeholder="Set a secure password" value={newUser.password} onChange={e => setNewUser(u => ({...u, password: e.target.value}))}/>
            </div>
            <div style={{ marginBottom:22 }}>
              <label style={s.label}>Starting plan</label>
              <select style={s.input} value={newUser.plan} onChange={e => setNewUser(u => ({...u, plan: e.target.value}))}>
                <option value="free">Free — 100 credits</option>
                <option value="basic">Basic — 200 credits</option>
                <option value="pro">Pro — 600 credits</option>
                <option value="max">Max — 1,500 credits</option>
              </select>
            </div>

            <div style={{ display:'flex', gap:10 }}>
              <button
                style={{ flex:1, padding:'10px', background:'var(--blue)', color:'#fff', border:'none', borderRadius:8, fontSize:14, fontWeight:600, cursor:'pointer', boxShadow:'0 0 12px var(--blue-glow)', fontFamily:'inherit', opacity: addingUser ? 0.6 : 1 }}
                onClick={createUser}
                disabled={addingUser}
              >
              ...
              <button
                style={{ padding:'10px 18px', background:'transparent', color:'var(--text-secondary)', border:'1px solid var(--border)', borderRadius:8, fontSize:14, cursor:'pointer', fontFamily:'inherit' }}
      ...
      <div style={s.main}>
        <div style={s.topbar}>
          ...
          <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
            ...
            {activeTab === 'users' && <button style={{ ...s.refreshBtn, background:'var(--blue)', color:'#fff', border:'1px solid var(--blue)', boxShadow:'0 0 12px var(--blue-glow)' }} onClick={() => setShowAddUser(true)}>+ Add user</button>}
...
const s = {
  ...
  modal:        { background:'var(--bg-card)', border:'1px solid var(--border-bright)', borderRadius:14, padding:28, width:'100%', maxWidth:420, boxShadow:'0 0 48px var(--blue-glow)' },
  label:        { display:'block', fontSize:13, fontWeight:500, color:'var(--text-secondary)', marginBottom:5 },
  input:        { width:'100%', padding:'9px 12px', background:'var(--bg-input)', border:'1px solid var(--border)', borderRadius:8, color:'var(--text-primary)', fontSize:14, fontFamily:'inherit', outline:'none', marginBottom:0 },
}
